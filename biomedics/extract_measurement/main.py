...
def _normalise_unit(unit):
    ...
    match_dfg_unit = re.findall(
        r"(ml\s?\/\s?mi?n\s?\/\s?1[.,]73\s?m2|ml\s?\/\s?mi?n\s?\/\s?m2)",
...
def match_date_pattern(df):
    ...
    df_with_dates = df_with_dates.withColumn(
        ...
        F.regexp_replace("lexical_variant_stripped", date_pattern, ""),
    )
...
def clean_lexical_variant(df):
    ...
    df = df.withColumn(
        "lexical_variant_stripped",
        F.regexp_replace("lexical_variant_stripped", r"⁹", " "),
    )
    df = df.withColumn(
        "lexical_variant_stripped",
        F.regexp_replace("lexical_variant_stripped", r"m²", "m2"),
    )
    df = df.withColumn(
        "lexical_variant_stripped",
        F.regexp_replace("lexical_variant_stripped", r"m³", "m3"),
    )
    ...
    df = df.withColumn(
        "lexical_variant_stripped",
        F.regexp_replace("lexical_variant_stripped", r"à", "="),
    )
    df = df.withColumn(
        "lexical_variant_stripped",
        F.regexp_replace("lexical_variant_stripped", r"(\:)", " "),
    ...
    df_biocomp_bio_clean = df.withColumn(
        "lexical_variant_stripped",
        clean_lexical_variant_udf(df["lexical_variant_stripped"]),
    )
...
def extract_clean_units(df):
    ...
    df = df.withColumn(
        "lexical_variant_stripped", clean_units_udf(df["lexical_variant_stripped"])
    )
    ...
    df = df.withColumn(
        "lexical_variant_stripped",
        F.regexp_replace(
            "lexical_variant_stripped", pattern_digit_nospace_wt_unit, r"$1 $2"
        ),
    )
    ...
    df_units = df_units.withColumn(
        "lexical_variant_stripped",
        F.regexp_replace("lexical_variant_stripped", units, " "),
    )
...
def extract_clean_values(df):
    ...
    df_values = df_values.withColumn(
        "value_cleaned", clean_values_udf(df_values["value"])
    )
...
def bio_post_processing(spark, script_config, brat_dir, output_dir):
    ...
    logger.info(
        "---------------Select columns of interest and convert to Pandas---------------"
    )
    ...
    try:
        parquet_path = "hdfs://bbsedsi/user/cse200093/temp.parquet"
