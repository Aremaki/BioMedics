...
def run(spark, sql, config):
    ...
    for input_dir, output_dir in zip(input_dirs, output_dirs):
        ...
        outcome_df = sql(
            """SELECT doc.instance_num, cim10.concept_cd AS icd10_codes, doc.encounter_num, doc.patient_num, visit.age_visit_in_years_num, visit.start_date, visit.mode_sortie, visit.length_of_stay, patient.death_date FROM i2b2_observation_doc AS doc JOIN i2b2_visit AS visit ON doc.encounter_num = visit.encounter_num JOIN i2b2_patient AS patient ON doc.patient_num = patient.patient_num JOIN i2b2_observation_cim10 AS cim10 ON doc.encounter_num = cim10.encounter_num
